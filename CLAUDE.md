...
- **Correo**: SMTP saliente (Mailpit en desarrollo).
...
│   └── worker/        # Workers asíncronos en ARQ para extracción, Playwright y envío
├── packages/
│   └── shared/        # Tipos compartidos, esquemas de validación y constantes
├── infra/
│   ├── docker-compose.yml  # Servicios locales: Postgres 16, Redis 7, MinIO, Mailpit
│   └── postgres/
...
4. **El CFDI nunca se persiste:**
   Si el motor de facturación descarga el XML o el PDF del CFDI, estos se envían inmediatamente por SMTP al correo configurado en el perfil fiscal del usuario. El buffer en memoria se descarta dentro del mismo scope de ejecución. **Solo se persiguen el UUID fiscal del SAT y la dirección de correo a donde se envió.** No existe endpoint ni almacenamiento para descargar facturas.
